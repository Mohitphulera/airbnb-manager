...
function compressImage(file: File): Promise<File> {
  return new Promise((resolve) => {
    ...
    img.onerror = () => {
      URL.revokeObjectURL(url)
      // If Image() can't decode (HEIC on Chrome), upload raw — Cloudinary will convert server-side
      if (isHEIC) {
        console.warn('Browser cannot decode HEIC, uploading raw for server-side conversion')
      }
      resolve(file)
    }
    img.src = url
